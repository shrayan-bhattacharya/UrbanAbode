    ...
    const fetchData = async () => {
      ...
      if (error) {
        ...
        const fetchedProperties = data.map((p: any) => ({
          ...
          price: String(p.price || "Price on request"), // Ensure price is a string
          location: p.location,
          imageUrl: p.image_url, 
          videoUrl: p.video_url, 
          createdAt: p.created_at,
          bedrooms: p.bhk || p.bedrooms, 
          area: String(p.area || "N/A"), // Ensure area is a string
          rera_id: p.rera_id,
        })) as Property[];
    ...
    <div className="bg-background text-foreground">
      <section ref={heroRef} className="relative min-h-[70vh] md:min-h-screen flex items-center justify-center text-center px-4 z-0 overflow-hidden">
        ...
        <div ref={overlayRef} className="relative z-10 p-6 sm:p-8 glassmorphism-deep max-w-3xl mx-auto">
          <h1 ref={headingRef} className="text-4xl font-extrabold tracking-tight text-white sm:text-5xl md:text-6xl drop-shadow-md"> 
            Discover Your <span className="text-accent dark:text-accent">Dream Home</span> 
          </h1>
          <p ref={subheadingRef} className="mt-6 max-w-xl mx-auto text-lg text-gray-200 sm:text-xl md:text-2xl drop-shadow-sm"> 
                  ...
                  <div className="glassmorphism-deep rounded-lg overflow-hidden shadow-xl flex flex-col h-full transition-all duration-300 ease-in-out group-hover:shadow-2xl group-hover:scale-105">
                    <div className="relative w-full h-60">
                      <Image
                        ...
                        data-ai-hint="property residence"
                      />
                    </div>
                    <div className="p-6 flex flex-col flex-grow">
                      ...
                      <p className="font-bold text-lg text-accent mt-auto">
                        {property.price} {/* Display price string directly */}
                      </p>
                    </div>
                  </div>
                </Link>
